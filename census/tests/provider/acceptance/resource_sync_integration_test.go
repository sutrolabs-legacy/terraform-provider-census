...
// TestAccResourceSync_ComputedDefaults tests that alert and run_mode fields
// that are computed by the API don't cause drift when not explicitly defined
func TestAccResourceSync_ComputedDefaults(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:  func() { provider_test.TestAccPreCheckIntegration(t) },
		Providers: provider_test.TestAccProviders,
		Steps: []resource.TestStep{
			{
				Config: testAccResourceSyncConfig_computedDefaults(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("census_sync.test", "label", "Test Computed Defaults"),
					resource.TestCheckResourceAttr("census_sync.test", "paused", "true"),
					// Verify API-set defaults are present in state
					resource.TestCheckResourceAttrSet("census_sync.test", "alert.#"),
					resource.TestCheckResourceAttrSet("census_sync.test", "run_mode.#"),
				),
			},
			{
				// Update only the label - alert and run_mode should persist without drift
				Config: testAccResourceSyncConfig_computedDefaultsUpdated(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("census_sync.test", "label", "Updated Computed Defaults"),
					resource.TestCheckResourceAttr("census_sync.test", "paused", "true"),
					// Verify alert and run_mode still exist and weren't deleted
					resource.TestCheckResourceAttrSet("census_sync.test", "alert.#"),
					resource.TestCheckResourceAttrSet("census_sync.test", "run_mode.#"),
				),
			},
...
func testAccResourceSyncConfig_computedDefaults() string {
	return testAccIntegrationBaseConfig() + `
resource "census_sync" "test" {
  workspace_id = census_workspace.test.id
  label        = "Test Computed Defaults"
  ...
  # NOTE: alert and run_mode are intentionally NOT defined here
  # The Census API will set defaults, and we want to verify they don't cause drift
...
func testAccResourceSyncConfig_computedDefaultsUpdated() string {
	return testAccIntegrationBaseConfig() + `
resource "census_sync" "test" {
  workspace_id = census_workspace.test.id
  label        = "Updated Computed Defaults"
  ...
  # NOTE: alert and run_mode are still intentionally NOT defined
  # We're testing that updating only the label doesn't cause drift for these fields
